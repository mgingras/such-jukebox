(function() {
	...
	Player = function(isHost) {
		...
		function populateSoundCloudInfoToQueuedSong(queuedSong, callback) {
			...
			SC
					.get(
							...
							function(track) {
								console
										.log('Got response soundcloud call for song ['
												+ queuedSong.id + ']');
								console.log(track);
			...
			if (isPlaying) {
				currentSong.sound.play({
					whileplaying : function() {
						that.updateTimeOfCurrentSong(this.position,
								this.duration);
					}
				});
			} else {
		...
		function getQueuedHtmlForSong(song) {
			...
			var html = '<li class="list-group-item queued-song-item" id="queued-song_'
					+ song.id
					+ '" data-song-queue-id="'
					+ song.id
					+ '">'
					+ '<div class="song-info-list">'
					+ '<div class="song-icon song-icon-list" id="queued-song_'
					+ song.id
					+ '_image" style="background-image: url('
					+ imageUrl
					+ ')"></div>'
					...
					+ '<a onclick="javascript: player.voteSong('
					+ song.id
					+ ', false)" class="btn btn btn-success btn-xs vote-song-up-btn '
					+ voteDisabled
					+ '" style="margin-bottom: 2px;"><span class="fa fa-thumbs-up"></span></a><br />'
					+ '<a onclick="javascript: player.voteSong('
					+ song.id
					+ ', true)" class="btn btn btn-danger btn-xs vote-song-down-btn '
					+ voteDisabled
					+ '"><span class="fa fa-thumbs-down"></span></a>'
					+ '</div>'
					+ '<a class="btn btn btn-default '
					+ buttonType
					+ ' btn-lg disabled song-votes-count"><span class="fa fa-thumbs-'
					+ upOrDown
					+ '">'
		...
		function getSearchResultHtmlForSong(song) {
			...
			var html = '<li class="list-group-item queued-song-item" id="search-result-song_'
					+ song.trackid
					+ '" data-song-track-id="'
					+ song.trackid
					+ '">'
					+ '<div class="song-info-list">'
					+ '<div class="song-icon song-icon-list" id="search-result_'
					+ song.trackid
					+ '_image" style="background-image: url('
					+ imageUrl
					+ ')"></div>'
					+ '<div class="song-details song-details-list">'
					+ '<p id="search-result-song_'
					+ song.trackid
					+ '_title">'
					+ title
					+ '</p>'
		...
		function updateSongInfoUIForQueuedSong(queuedSong) {
			$('#queued-song_' + queuedSong.id + '_title').text(
					queuedSong.song.title);
			$('#queued-song_' + queuedSong.id + '_artist').text(
					queuedSong.song.artist);
			$('#queued-song_' + queuedSong.id + '_image').css({
				'background-image' : 'url(' + queuedSong.song.imageUrl + ')'
			});
		}

		function updateSongProgressUI() {
			$('#current-song-progress > .progress-bar').css({
				width : currentSongPercentDone + '%'
			});
		}
		...
		function updateVotesToSkipGui() {
			if (currentSong !== undefined) {
				...
				$('#song-votes-to-skip').text(
						currentSong.numVotesToSkip + ' votes to skip')
			}
		}
		...
		this.voteSong = function(songQueueId, isVoteDown) {
			...
			$('#queued-song_' + songQueueId).find('.vote-song-up-btn')
					.addClass('disabled');
			$('#queued-song_' + songQueueId).find('.vote-song-down-btn')
					.addClass('disabled');
		...
		function handleNewPartyState(party) {
			...
			for ( var i in queuedSongs) {
				...
				if (cachedSong) {
					...
					populateSoundCloudInfoToQueuedSong(qs,
							updateSongInfoUIForQueuedSong);
				}
			}

			if (party.currentSong !== undefined && currentSong != undefined
					&& party.currentSong.id !== currentSong.id
					&& getQueuedSongById(party.currentSong.id) !== undefined) {
				handleChangeToSong(party.currentSong);
			} else if (party.currentSong !== undefined
					&& currentSong != undefined) {
				currentSong.numVotesToSkip = party.currentSong.numVotesToSkip;
			} else if (currentSong == undefined && queuedSongs.length > 0) {
				handleChangeToSong(queuedSongs[0]);
			}
		...
		function addMoreSongs() {
			...
			var randomLetter = ''
					+ alphabet.charAt(getRandom(0, alphabet.length - 1));
		...
		/*
		 * Perform on creation class="popover-content" style="color:#00B7FF;
		 */
		...
		var htmlForInviteOthers = '<div class="popover-content"><div class="share-box">'
				...
				+ // col
				'<div class="col-xs-4">'
				+ '<a href="https://twitter.com/intent/tweet?text=Start%20controlling%20the%20playlist%20for%20my%20party!%20Go%20to:&url='
				+ encodeURIComponent(window.location.toString())
				+ '" target="_blank">'
				...
				+ // col
				'<div class="col-xs-4">'
				+ '<a href="mailto:?subject=Join%20my%20party%20at%20Such%20Jukebox!!!&body=Start%20controlling%20the%20playlist%20for%20my%20party!%20Go%20to:%20'
